...
async function loadRewardTiers(){
  const { data, error } = await sb.from('reward_tiers').select('*').order('sort').order('created_at');
  if(error){ console.warn('loadRewardTiers', error); return; }
  state.rewardTiers = data || [];
}

async function loadRewards(){
  const { data, error } = await sb.from('rewards').select('*').order('sort').order('created_at');
  if(error){ console.warn('loadRewards', error); return; }
  state.rewards = data || [];
}

async function loadRedemptions(){
  const { data, error } = await sb.from('reward_redemptions').select('*').order('created_at', { ascending: false });
  if(error){ console.warn('loadRedemptions', error); return; }
...
async function loadCleaningTasks(){
  const { data, error } = await sb.from('cleaning_tasks').select('*').order('weekday').order('sort').order('created_at');
  if(error){ console.warn('loadCleaningTasks', error); return; }
  state.cleaningTasks = data || [];
}

async function loadCleaningDone(){
  const { data, error } = await sb.from('cleaning_done').select('*');
  if(error){ console.warn('loadCleaningDone', error); return; }
...
async function loadSchoolClosures(){
  const { data, error } = await sb.from('school_closures').select('*').order('start_date');
  if(error){ console.warn('loadSchoolClosures', error); return; }
  state.schoolClosures = data || [];
}
...
async function loadMeetings(){
  const { data, error } = await sb.from('meetings').select('*').order('weekday').order('date').order('start_time');
  if(error){ console.warn('loadMeetings', error); return; }
  state.meetings = data || [];
}
